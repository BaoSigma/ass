...
public interface PhieuBanHangDAO extends CRUDDAO<PhieuBanHang, Object> {    
    ...
    @Override
    public default PhieuBanHang create(PhieuBanHang Pbh){
        list.add(Pbh);
        return Pbh;
    }
    ...
    public default void deleteById(Object id){
        list.removeIf(Pbh -> Pbh.getMaPhieu().equals(id));
    }
    ...
    public default PhieuBanHang findById(Object id) {
        Optional<PhieuBanHang> PhieuBanHang = list.stream().filter(Pbh -> Pbh.getMaPhieu().equals(id)).findFirst();
